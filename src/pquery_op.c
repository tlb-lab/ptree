/*
 * op function for ptree and pquery
 * Teodor Sigaev <teodor@stack.net>
 * contrib/ptree/pquery_op.c
 */
#include "postgres.h"
...
bool
			compare_subnode(ptree_level *t, char *qn, int len, int (*cmpptr) (const char *, const char *, size_t), bool anyend)
{
...
int
ptree_strncasecmp(const char *a, const char *b, size_t s)
{
...
static bool
checkLevel(pquery_level *curq, ptree_level *curt)
{
	int			(*cmpptr) (const char *, const char *, size_t);
	pquery_variant *curvar = LQL_FIRST(curq);
	int			i;

	for (i = 0; i < curq->numvar; i++)
	{
		cmpptr = (curvar->flag & LVAR_INCASE) ? ptree_strncasecmp : strncmp;
...
static bool
checkCond(pquery_level *curq, int query_numlevel, ptree_level *curt, int tree_numlevel, FieldNot *ptr)
{
	uint32		low_pos = 0,
				high_pos = 0,
				cur_tpos = 0;
	int			tlen = tree_numlevel,
				qlen = query_numlevel;
	int			isok;
	pquery_level *prevq = NULL;
	ptree_level *prevt = NULL;
...
ltq_regex(PG_FUNCTION_ARGS)
{
	...
	if (query->flag & LQUERY_HASNOT)
	{
		...
		res = checkCond(LQUERY_FIRST(query), query->numlevel,
						PTREE_FIRST(tree), tree->numlevel, &fn);
	}
	else
	{
		res = checkCond(LQUERY_FIRST(query), query->numlevel,
						PTREE_FIRST(tree), tree->numlevel, NULL);
	}
