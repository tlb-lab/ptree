...
PG_FUNCTION_INFO_V1(_ptree_compress);
Datum		_ptree_compress(PG_FUNCTION_ARGS);

PG_FUNCTION_INFO_V1(_ptree_same);
Datum		_ptree_same(PG_FUNCTION_ARGS);

PG_FUNCTION_INFO_V1(_ptree_union);
Datum		_ptree_union(PG_FUNCTION_ARGS);

PG_FUNCTION_INFO_V1(_ptree_penalty);
Datum		_ptree_penalty(PG_FUNCTION_ARGS);

PG_FUNCTION_INFO_V1(_ptree_picksplit);
Datum		_ptree_picksplit(PG_FUNCTION_ARGS);

PG_FUNCTION_INFO_V1(_ptree_consistent);
Datum		_ptree_consistent(PG_FUNCTION_ARGS);
...
Datum
_ptree_compress(PG_FUNCTION_ARGS)
{
	...
	{							/* ptree */
		...
		ptree	   *item = (ptree *) ARR_DATA_PTR(val);
...
static int4
unionkey(BITVECP sbase, ptree_gist *add)
{
	int4		i;
...
static int
hemdist(ptree_gist *a, ptree_gist *b)
{
	if (LTG_ISALLTRUE(a))
	{
		if (LTG_ISALLTRUE(b))
...
Datum
_ptree_penalty(PG_FUNCTION_ARGS)
{
...
Datum
_ptree_picksplit(PG_FUNCTION_ARGS)
{
	GistEntryVector *entryvec = (GistEntryVector *) PG_GETARG_POINTER(0);
	...
	{
		datum_l = (ptree_gist *) palloc(LTG_HDRSIZE);
		SET_VARSIZE(datum_l, LTG_HDRSIZE);
		datum_l->flag = LTG_ALLTRUE;
	}
	else
	{
		datum_l = (ptree_gist *) palloc(LTG_HDRSIZE + ASIGLEN);
	...
	{
		datum_r = (ptree_gist *) palloc(LTG_HDRSIZE);
		SET_VARSIZE(datum_r, LTG_HDRSIZE);
		datum_r->flag = LTG_ALLTRUE;
	}
	else
	{
		datum_r = (ptree_gist *) palloc(LTG_HDRSIZE + ASIGLEN);
...
static bool
gist_te(ptree_gist *key, ptree *query)
{
	ptree_level *curq = PTREE_FIRST(query);
	...
	while (qlen > 0)
	{
		hv = ptree_crc32_sz(curq->name, curq->len);
...
static bool
_arrq_cons(ptree_gist *key, ArrayType *_query)
{
...
Datum
_ptree_consistent(PG_FUNCTION_ARGS)
{
	GISTENTRY  *entry = (GISTENTRY *) PG_GETARG_POINTER(0);
	...
	*recheck = true;
	...
	switch (strategy)
	{
		case 10:
		case 11:
			res = gist_te(key, (ptree *) query);
			break;
		case 12:
		case 13:
			res = gist_qe(key, (pquery *) query);
			break;
		case 14:
		case 15:
