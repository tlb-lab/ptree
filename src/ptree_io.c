...
PG_FUNCTION_INFO_V1(ptree_in);
Datum		ptree_in(PG_FUNCTION_ARGS);

PG_FUNCTION_INFO_V1(ptree_out);
Datum		ptree_out(PG_FUNCTION_ARGS);

PG_FUNCTION_INFO_V1(pquery_in);
Datum		pquery_in(PG_FUNCTION_ARGS);

PG_FUNCTION_INFO_V1(pquery_out);
Datum		pquery_out(PG_FUNCTION_ARGS);
...
ptree_in(PG_FUNCTION_ARGS)
{
	...
	while (*ptr)
	{
		...
		if (charlen == 1 && t_iseq(ptr, PTREE_SEPARATOR))
			num++;
		ptr += charlen;
	}
	...
	result = (ptree *) palloc0(PTREE_HDRSIZE + totallen);
	SET_VARSIZE(result, PTREE_HDRSIZE + totallen);
	result->numlevel = lptr - list;
	curlevel = PTREE_FIRST(result);
	lptr = list;
...
#define NEXTLEV(x) ( (pquery_level*)( ((char*)(x)) + ITEMSIZE) )

Datum
pquery_in(PG_FUNCTION_ARGS)
{
	char	   *buf = (char *) PG_GETARG_POINTER(0);
	char	   *ptr;
	int			num = 0,
	...
	num++;
	curqlevel = tmpql = (pquery_level *) palloc0(ITEMSIZE * num);
	ptr = buf;
	while (*ptr)
	{
		charlen = pg_mblen(ptr);
		...
		else if (state == LQPRS_WAITDELIM)
		{
			...
			else if (charlen == 1 && t_iseq(ptr, PTREE_SEPARATOR))
			{
				lptr->len = ptr - lptr->start -
					((lptr->flag & LVAR_SUBLEXEME) ? 1 : 0) -
					((lptr->flag & LVAR_INCASE) ? 1 : 0) -
	...
	result = (pquery *) palloc0(totallen);
	...
	while ((char *) curqlevel - (char *) tmpql < num * ITEMSIZE)
	{
		...
		if (curqlevel->numvar)
		{
			...
			while (lptr - GETVAR(curqlevel) < curqlevel->numvar)
			{
				...
				lrptr->val = ptree_crc32_sz(lptr->start, lptr->len);
...
Datum
pquery_out(PG_FUNCTION_ARGS)
{
	pquery	   *in = PG_GETARG_LQUERY(0);
	char	   *buf,
			   *ptr;
	int			i,
				j,
				totallen = 1;
	pquery_level *curqlevel;
	pquery_variant *curtlevel;
